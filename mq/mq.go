...
type IMessageQueue interface {
	...
	XAddExclusiveMessage(topic string, values map[string]interface{}) error
	XRegisterConsumer(topics []string, handler func(error, string, *XMessage) error)
	XRegisterExclusiveConsumer(topics []string, consumer string, handler func(error, string, *XMessage) error)
